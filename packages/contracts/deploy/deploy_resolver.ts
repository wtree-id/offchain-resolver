...
const deployFunction: DeployFunction = async ({ deployments, ethers }: HardhatRuntimeEnvironment) => {
  const signers = await ethers.getSigners();
  const owner = signers[0].address;
  const registryDeploymentAddress = await deployments.getOrNull('ENSRegistry').then((d) => d?.address);
  const resolverDeploymentAddress = await deployments.getOrNull('OffchainResolver').then((d) => d?.address);
